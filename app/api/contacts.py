...
# Schema para a requisição de sincronização
...
@router.post("/sync/csv", summary="Sincronizar contatos de uma planilha Google Sheets (CSV)")
async def sync_contacts_from_sheet(
    ...
    try:
        contacts_from_sheet = await sheets_service.get_csv_sheet_as_json(request.url)
        
        summary = await crud_contact.create_or_update_contacts(
            db, 
            contacts_in=contacts_from_sheet, 
